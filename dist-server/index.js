...
var BUCKET_NAME = process.env.MINIO_BUCKET_NAME;
...
var uploadMiddleware = multer({
  ...
  limits: {
    ...
    // Batasan ukuran file (contoh: maksimal 5MB)
  },
...
var JWT_SECRET = process.env.JWT_SECRET || "heworqcv89y47ry34bcti4ycit438obvi74b57li6v534v13vy54y56";
...
var handleSuccess = (res, data, message) => res.json({ success: true, data, message });
var handleError = (res, error, status = 500) => {
  console.error(error);
...
app.post("/api/upload", requireAuth, (req, res, next) => {
  ...
  try {
    ...
    await minioClient.putObject(
      ...
      objectName,
      req.file.buffer,
      // Datanya dikirim sebagai Memory Buffer (ram)
      req.file.size,
...
app.put("/api/users/:id", requireAuth, requireAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    const { name, email, password, role, nip } = req.body;
    const data = { name, email, role, nip: nip || null };
    if (password) {
...
var generateUniqueSlug = async (title, excludeId) => {
...
app.put("/api/posts/:id", requireAuth, async (req, res) => {
  try {
    ...
    const post = await prisma.post.update({
      where: { id },
      data: { ...data, category, updatedAt: /* @__PURE__ */ new Date() }
    });
...
app.get("/api/graduations", requireAuth, async (req, res) => {
  try {
    ...
    const sortOrder = req.query.sortOrder || "desc";
    const searchNisn = req.query.searchNisn;
...
var generateUniqueEkskulSlug = async (title, excludeId) => {
  const baseSlug = title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)+/g, "");
  let slug = baseSlug;
  let counter = 1;
  const where = excludeId ? { NOT: { id: excludeId } } : {};
...
app.get("/api/holidays", requireAuth, async (req, res) => {
  try {
    ...
    const formatted = holidays.map((h) => ({ ...h, created_at: h.createdAt, updated_at: h.updatedAt }));
    handleSuccess(res, formatted);
  } catch (error) {
    handleError(res, error);
  }
